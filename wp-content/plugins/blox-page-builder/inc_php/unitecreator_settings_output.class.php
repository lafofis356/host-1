...
defined('BLOXBUILDER_INC') or die('Restricted access');
	...
	class UniteCreatorSettingsOutput extends UniteSettingsOutputUC{
		...
		private function a_______COLS_LAYOUT__________(){}
		...
		protected function drawColsLayoutInput($setting){
			...
			?>
				...
				<div id="<?php echo $id?>" data-name="<?php echo $name?>" data-settingtype="col_layout" class="uc-setting-cols-layout unite-setting-input-object">
		...
		private function a_______SAVE_GRID_PANEL__________(){}
		...
		private function drawSaveGridPanelButton($setting){
			...
			?>
			<div id="<?php echo $id?>" data-name="<?php echo $name?>" data-settingtype="save_section_tolibrary" class="uc-setting-save-panel-wrapper unite-setting-input-object">
				
				<?php 
				$buttonTitle = __("Save Section", BLOXBUILDER_TEXTDOMAIN);
				$loaderTitle = __("Saving...", BLOXBUILDER_TEXTDOMAIN);
				$successTitle = __("Section Saved", BLOXBUILDER_TEXTDOMAIN);
		...
		private function a_______GRID_PANEL_BUTTON__________(){}
		...
		private function drawGridPanelButton($setting){
			...
			if(empty($label))
				$label = __("Click Me", BLOXBUILDER_TEXTDOMAIN);
			...
			?>
			<div id="<?php echo $id?>" data-settingtype="grid_panel_button" class="unite-setting-input-object uc-grid-panel-button-wrapper">
				
				<a id="<?php echo $id?>_button" data-action="<?php echo $action?>" <?php echo $addHtml?> href="javascript:void(0)"  class="uc-grid-panel-button <?php echo $class?>"><?php echo $label?></a>
		...
		private function a_______SIZE_RELATED_LAYOUT__________(){}
		...
		protected function drawSizeInput_label($setting, $size){
			...
			<div class="unite-setting-text uc-tip <?php echo $addClass?>" title="<?php echo $description?>">
				<?php echo $label?>
			</div>
		...
		protected function drawFourInputsInput_size($setting, $baseName, $size, $arrSuffix, $arrTitles){
			...
			<div class="unite-setting-paddingline unite-clear <?php echo $addClass?>">
				<?php 
		...
		protected function drawFourInputsInput($setting){
			...
			$arrTitles = array(
					__("Top",BLOXBUILDER_TEXTDOMAIN),
					__("Right",BLOXBUILDER_TEXTDOMAIN), 
					__("Bottom",BLOXBUILDER_TEXTDOMAIN),
					__("Left",BLOXBUILDER_TEXTDOMAIN)
			);
		...
		protected function drawInputWithSizes($setting){
			...
			foreach($arrSizes as $size){
				...
				<div class="<?php echo $showinClass?>">
		...
		private function a_______IMAGE_AND_MP3__________(){}
		...
		protected function drawImageAddonInput($setting){
			...
			?>
				<div class="unite-setting-image <?php echo $addClass?>"> 
					...
					<div class='unite-setting-image-error' <?php echo $errorStyle?>><?php _e("Please select assets path", BLOXBUILDER_TEXTDOMAIN)?></div>
		...
		protected function drawMp3AddonInput($setting){
			...
			?>
				<div class="unite-setting-mp3">
					...
					<a href="javascript:void(0)" class="unite-button-secondary unite-button-choose <?php echo $buttonAddClass?>"><?php _e("Choose", BLOXBUILDER_TEXTDOMAIN)?></a>
					<div class='unite-setting-mp3-error' <?php echo $errorStyle?>><?php _e("Please select assets path", BLOXBUILDER_TEXTDOMAIN)?></div>
				</div>
			<?php
		}
		...
		private function getFontsPanelHtmlFields($arrParams, $arrFontsData, $addTemplate = false){
			...
			foreach ($arrParams as $name => $title):
				 ...
				 if($addTemplate == true)
				 	$selectFontTemplate = HelperHtmlUC::getHTMLSelect($arrFontsTemplate, $fontTemplate,"data-fieldname='template' class='{$classInput}'", true, "not_chosen", __("---- Select Page Font----", BLOXBUILDER_TEXTDOMAIN));
				 
				 $selectFontFamily = HelperHtmlUC::getHTMLSelect($arrData["arrFontFamily"],$fontFamily,"data-fieldname='font-family' class='{$classInput}'", true, "not_chosen", __("Select Font Family", BLOXBUILDER_TEXTDOMAIN));
				 
				 $selectFontWeight = HelperHtmlUC::getHTMLSelect($arrData["arrFontWeight"],$fontWeight,"data-fieldname='font-weight' class='{$classInput}'", false, "not_chosen", __("Select", BLOXBUILDER_TEXTDOMAIN));
				 
				 $selectLineHeight = HelperHtmlUC::getHTMLSelect($arrData["arrLineHeight"],$lineHeight,"data-fieldname='line-height' class='{$classInput}'", false, "not_chosen", __("Select", BLOXBUILDER_TEXTDOMAIN));
				 $selectTextDecoration = HelperHtmlUC::getHTMLSelect($arrData["arrTextDecoration"],$textDecoration,"data-fieldname='text-decoration' class='{$classInput}'", false, "not_chosen", __("Select Text Decoration", BLOXBUILDER_TEXTDOMAIN));
				 
				 $selectFontStyle = HelperHtmlUC::getHTMLSelect($arrData["arrFontStyle"],$fontStyle,"data-fieldname='font-style' class='{$classInput}'", false, "not_chosen", __("Select", BLOXBUILDER_TEXTDOMAIN));
			     ...
			     $text = __("Font Size - Mobile (px)", BLOXBUILDER_TEXTDOMAIN);
			     ...
			     $html .= "		".__("Line Height", BLOXBUILDER_TEXTDOMAIN)."<br>".$br;
			     ...
			     $html .= "<div class=\"uc-fontspanel-line\">".$br;
		      	 ...
		      	 $html .= "	".__("Text Decoration", BLOXBUILDER_TEXTDOMAIN)."<br>".$br;
		      	 $html .= $selectTextDecoration;
		      	 $html .= "</span>".$br;
			     
		      	 $html .= "<span class=\"{$classSection} uc-details-color\">".$br;
		      	 $html .= "	".__("Color", BLOXBUILDER_TEXTDOMAIN)."<br>".$br;
		...
		public function getFontsParams($arrFontNames, $arrFontsData){
			...
			foreach($arrFontNames as $name => $title){
				...
				$fontSizeTablet = UniteFunctionsUC::getVal($fontData, "font-size-tablet",$valueNotChosen);
				$fontSizeMobile = UniteFunctionsUC::getVal($fontData, "mobile-size",$valueNotChosen);
				...
				$arrFields[] = $this->getFontsParams_getArrParam(UniteCreatorDialogParam::PARAM_TEXTFIELD, "font-size", 	$name, "Font Size", $fontSize);
				$arrFields[] = $this->getFontsParams_getArrParam(UniteCreatorDialogParam::PARAM_TEXTFIELD, "font-size-tablet", 	$name, "Font Size - Tablet", $fontSizeTablet);
				$arrFields[] = $this->getFontsParams_getArrParam(UniteCreatorDialogParam::PARAM_DROPDOWN, "mobile-size", 	$name, "Font Size - Mobile", $fontSizeMobile);
		...
		protected function drawFontsPanel($setting){
			...
			if(empty($arrParamsNames)){
				...
				$html .= "<h2>".__("Edit Fonts", BLOXBUILDER_TEXTDOMAIN)."</h2>";
		...
		protected function drawItemsPanel($setting){
			...
			<?php 
				...
				if($this->isSidebar == true): ?>
					<a href="javascript:void(0)" class="unite-button-secondary uc-setting-items-panel-button"><?php _e("Edit Addon Items", BLOXBUILDER_TEXTDOMAIN)?></a>
					
					<div id='<?php echo $idDialog?>' class='uc-settings-items-panel-dialog' title="<?php _e("Edit Addon Items", BLOXBUILDER_TEXTDOMAIN)?>" style='display:none'>
				<?php endif;
