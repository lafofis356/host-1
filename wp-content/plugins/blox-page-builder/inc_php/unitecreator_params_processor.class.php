...
class UniteCreatorParamsProcessorWork{
	...
	* return if it's output process type
	...
	private function getParamsNamesForFonts($paramsType){
		...
		foreach($arrParams as $param){
			...
			if($paramsType == "items"){
				...
				$title = __("Items", BLOXBUILDER_TEXTDOMAIN)." => ".$title;
			}
	...
	private function ____________ICON______________(){}
	...
	private function ____________SHAPE______________(){}
	...
	private function a________MENU______(){}
	...
	private function getMenuData($value, $name, $param, $processType){
		...
		$messageEmpty = __("No menu selected", BLOXBUILDER_TEXTDOMAIN);
		...
		if(empty($arrItems)){
			$message = __("No items in ",BLOXBUILDER_TEXTDOMAIN).$menuType.__(" menu",BLOXBUILDER_TEXTDOMAIN);
			return($message);
		}
