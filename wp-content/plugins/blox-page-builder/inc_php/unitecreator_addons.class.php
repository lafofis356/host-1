...
class UniteCreatorAddons extends UniteElementsBaseUC{
	...
	protected function getCatsAndAddonsHtml($catID, $type){
		...
		$objManager = UniteCreatorManager::getObjManagerByAddonType($type);
	 ...
	 * get category items
	 */
	public function getCatAddons($catID, $isShort = false, $filterActive = null, $addonType = null, $includeImages = false){
	...
	public function createFromManager($data){
		...
		if($isLayout == false){
			...
			$newLayoutID = $objLayout->createSmall($title, $name, $description, $catID);
