...
class UniteCreatorGridBuilder{
	...
	public function initByLayout(UniteCreatorLayout $objLayout){
		...
		$this->arrParams = $objLayout->getParams();
	}
	 ...
	 * put js deviders
	 */
	protected function putArrJsDividers(){
		...
		$arrShapesDividers = $objShapes->getArrShapeDividers("data");
				
		$jsonDividers = UniteFunctionsUC::jsonEncodeForClientSide($arrShapesDividers);
		
		?>
		var g_ucShapeDividersJson = <?php echo $jsonDividers?>
	...
	private function putLayoutDialog(){
		...
		$titleText = __("Choose Row Layout", BLOXBUILDER_TEXTDOMAIN);
	...
	private function getBuilderOptions(){
		...
		$options["icons"] = $this->arrIcons;
		$options["params"] = $this->arrParams;
	...
	public function putGrid(){
		...
		?>
			<div class="uc-grid-builder-wrapper uc-builder-wrapper" >
