...
class UniteCreatorForm extends HtmlOutputBaseUC{
	...
	public function addFormSettings($objSettings, $name, $value, $title, $extra){
		...
		if(empty($adminEmail))
			$objSettings->addStaticText(__("Please fill admin email in",BLOXBUILDER_TEXTDOMAIN)." {$linkGeneralSettings}.");
		else
			$objSettings->addStaticText(__("Admin Email",BLOXBUILDER_TEXTDOMAIN).": <b>$adminEmail</b> (".__("you can change it in",BLOXBUILDER_TEXTDOMAIN)." {$linkGeneralSettings})");
		
		$objSettings->addHr();
		
		$sendTextValue = UniteFunctionsUC::getVal($arrValues, $name."_send_button_text", "Send");
		$objSettings->addTextBox($name."_send_button_text",$sendTextValue,__("Send Button Text", BLOXBUILDER_TEXTDOMAIN));
		
		$sendingTextValue = UniteFunctionsUC::getVal($arrValues, $name."_sending_text", "Sending...");
		$objSettings->addTextBox($name."_sending_text",$sendingTextValue,__("Loading Text", BLOXBUILDER_TEXTDOMAIN));
		
		$successTextValue = UniteFunctionsUC::getVal($arrValues, $name."_success_text", "Thank you for contacting us");
		$objSettings->addTextBox($name."_success_text",$successTextValue,__("Thank you text", BLOXBUILDER_TEXTDOMAIN));
	...
	public function getFormOutputData($data, $paramName, $arrValues){
		...
		$sendButtonText = UniteFunctionsUC::getVal($arrValues, "{$paramName}_send_button_text", __("Send", BLOXBUILDER_TEXTDOMAIN));
	...
	private function validateFieldRequired($arrField){
		...
		$message = __("Please fill",BLOXBUILDER_TEXTDOMAIN). " <b>".$title. "</b>". __(" field",BLOXBUILDER_TEXTDOMAIN);
