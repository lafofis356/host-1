...
defined('BLOXBUILDER_INC') or die('Restricted access');
	...
	class UniteCreatorAddonWork extends UniteElementsBaseUC{
		...
		private function validateName($name){
			...
			$fieldName = __("Addon Name", BLOXBUILDER_TEXTDOMAIN);
		...
		private function initParamsFindDoubles($arrParams){
			...
			foreach($arrParams as $key=>$param){
				...
				if(array_key_exists($name, $arrNames) == true)
					$arrParams[$key]["param_error"] = __("Double Name, please remove", BLOXBUILDER_TEXTDOMAIN);
		...
		public function getAdminLabels(){
			...
			if($this->isHasItems()){
				$this->arrAdminLabels[] = array("uc_num_items", __("Items", BLOXBUILDER_TEXTDOMAIN));
			}
		...
		private function a_____________GET_INCLUDES______________(){}
		...
		public function getHtmlConfig($putMode = false, $isOutputSidebar = false, $options = array()){
			...
			if(empty($this->params)){
				...
				$html = __("no settings for this addon", BLOXBUILDER_TEXTDOMAIN);
			...
			*/
			...
			if(!empty($this->params) || $this->hasItems){
				...
				$objSettings->addSap(__("General",BLOXBUILDER_TEXTDOMAIN),"config",true);
			...
			if(GlobalsUC::$inDev == true){
				...
				if($this->hasItems == true){
					$objSettings->addSap(__("Edit Items",BLOXBUILDER_TEXTDOMAIN),"items");
			...
			if($isFontsPanelEnabled == true){
				...
				$objSettings->addSap(__("Fonts",BLOXBUILDER_TEXTDOMAIN),"fonts");
			...
			if($numSettings == 0){
				$textEmpty = __("no settings for this addon", BLOXBUILDER_TEXTDOMAIN);
		...
		public function getArrFontsParams(){
			...
			$arrParams = $settingsOutput->getFontsParams($arrParamNames, $arrFontsData);
