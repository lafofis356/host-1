 ...
 */
	class HelperUC extends UniteHelperBaseUC{
		 ...
		 * is url under assets
		 */
		public static function isUrlUnderAssets($url){
			
			$url = self::URLtoFull($url);
			$url = strtolower($url);
			if(strpos($url, GlobalsUC::$url_assets) !== false)
				return(true);
		...
		public static function outputTemplatePart($layoutType){
			...
			if(empty($objLayout)){
				...
				$message = __("Template part",BLOXBUILDER_TEXTDOMAIN) .": ". $typeTitle .__(" not found. Please create one in template parts page", BLOXBUILDER_TEXTDOMAIN);
