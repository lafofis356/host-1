...
class UniteCreatorCategoriesWork extends UniteElementsBaseUC{
	...
	private function getListExtraLayouts($filterTitle="", $ordering=""){
		...
		$arrCatsNew[] = array(
				"id" => 0,
				"title" => __("Uncategorized", BLOXBUILDER_TEXTDOMAIN),
				"num_layouts" => 0
		);
	 ...
	 */
	protected function getListExtraQuery(UniteCreatorAddonType $objAddonType, $filterTitle="", $ordering=""){
	 ...
	 */
	public function getListExtra(UniteCreatorAddonType $objAddonType, $filterTitle="", $ordering="", $showAll = false){
		
		$query = $this->getListExtraQuery($objAddonType, $filterTitle, $ordering);
	...
	private function getFirstCatByAddType($addType){
		...
		switch($addType){
			case "empty":
				$arrCatsOutput[""] = __("[Not Selected]", BLOXBUILDER_TEXTDOMAIN);
			break;
			case "new":
				$arrCatsOutput["new"] = __("[Add New Category]", BLOXBUILDER_TEXTDOMAIN);
			break;
			case "component":
				$arrCatsOutput[""] = __("[From Gallery Settings]", BLOXBUILDER_TEXTDOMAIN);
			break;
			case "all_uncat":
	...
	public function getCatListFromData($data){
		...
		UniteCreatorManagerAddons::setStateFilterActive($filterActive);
		
		$htmlCats = $this->getHtmlCatList($selectedCat, $objAddonType);
	...
	private function getCatHTML($cat, $class = ""){
		...
		if($isweb == true){
			$webText = __("Web",BLOXBUILDER_TEXTDOMAIN);
	...
	*/
	...
	public function addFromData($data){
		...
		$response["message"] = __("Category Added", BLOXBUILDER_TEXTDOMAIN);
		$response["htmlSelectCats"] = $this->getHtmlSelectCats($type);
