...
class UniteCreatorSettingsWork extends UniteSettingsAdvancedUC{
	...
	* get multiple params creator format from one param
	...
	private function a________SETTINGS_TYPES________(){}
	...
	protected function addInstagramSelector($name,$value,$title,$extra){
		...
		$description = __("Type instagram user (@username)", BLOXBUILDER_TEXTDOMAIN);
		...
		$this->addTextBox($name, $value ,__("Instagram User", BLOXBUILDER_TEXTDOMAIN), $params);
	}
	...
	public function addImageBaseSettings(){
		...
		$this->addTextArea("description", "", __("Description", BLOXBUILDER_TEXTDOMAIN),$extra);
		
		/*
		$extra = array("origtype"=>"uc_radioboolean");
		$this->addRadioBoolean("enable_link", __("Enable Link", BLOXBUILDER_TEXTDOMAIN),false, "Yes","No",$extra);
			
		$extra = array("class"=>"unite-input-link", "origtype"=>"uc_textfield");
		$this->addTextBox("link", "", __("Link", BLOXBUILDER_TEXTDOMAIN),$extra);
		*/
	...
	protected function modifyBeforeAdd_categories($setting, $addonType, $addNew = false){
		...
		if($addNew == true)
			$arrCats["new"] = "[". __("New Category", BLOXBUILDER_TEXTDOMAIN)."]";
