function UniteCreatorAdmin_LayoutsList(){
	...
	this.initImportLayoutDialog = function(addParams, layoutID){
		...
		if(g_objButtonCatalogImport.length)
			g_objButtonCatalogImport.click(openImportPageCatalog);
	...
	function initEvents(){
		...
		if(g_tableLayouts){
			...
			g_tableLayouts.delegate(".button_duplicate", "click", t.onDuplicateClick);
	...
	function initEventsCats(){
		...
		*/
		...
		jQuery("#uc_dialog_add_category_action").click(setCategoryForLayout);
	...
	*/
	...
	function initManageCategoryDialog(){
		...
		jQuery("#uc_dialog_add_category_button_add").click(addCategory);
