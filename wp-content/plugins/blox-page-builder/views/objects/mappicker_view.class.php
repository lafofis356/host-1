...
class UniteCreatorMappickerView{
	...
	private function initSettings(){
		...
		$settings->addSap(__("Size and Location", BLOXBUILDER_TEXTDOMAIN));
		
		$settings->addTextBox("width","100%",__("Map Width",BLOXBUILDER_TEXTDOMAIN),array("unit"=>"px or %"));
		$settings->addTextBox("height","300",__("Map Height",BLOXBUILDER_TEXTDOMAIN),array("unit"=>"px or %"));
		...
		$settings->addButton("button_my_location", __("Goto My Location", BLOXBUILDER_TEXTDOMAIN), UniteSettingsUC::PARAM_NOTEXT, $params);
		
		//--------- add marker
		
		$settings->addSap(__("Marker", BLOXBUILDER_TEXTDOMAIN));
		...
		$settings->addImage("marker_image","",__("Marker Image",BLOXBUILDER_TEXTDOMAIN));
		$settings->addControl("marker_type", "marker_image", "show", "image");
		
		
		$settings->addSap(__("Style", BLOXBUILDER_TEXTDOMAIN));
		...
		$settings->addSap(__("Language and Controls", BLOXBUILDER_TEXTDOMAIN));
		
		//--- language
		$settings->addSelect("language", $this->arrLangs, __("Language", BLOXBUILDER_TEXTDOMAIN), "");
	...
	private function outputHtml(){
		...
		$linkGeneralSettings = HelperHtmlUC::getHtmlLink($urlGeneralSettings, __("General Settings",BLOXBUILDER_TEXTDOMAIN),"","",true);
		...
		?>	
			<div class="uc-mappicker-panels-wrapper">
				...
				<div class="uc-mappicker-overlay-text">
					<?php _e("For edit the map, please enter your google map API Key in", BLOXBUILDER_TEXTDOMAIN)?> <?php echo $linkGeneralSettings?>.
					<br>
					<br>
	...
	public function putPickerInputHtml(){
		...
		$status = __("No Map Chosen", BLOXBUILDER_TEXTDOMAIN);
		...
		?>
			<div class="unite-mappicker-chooser-wrapper">
	        	...
	        	<a href="javascript:void(0)" class="unite-mappicker-button unite-button-secondary unite-center-position" ><?php _e("Choose Map", BLOXBUILDER_TEXTDOMAIN)?></a>
			</div>
