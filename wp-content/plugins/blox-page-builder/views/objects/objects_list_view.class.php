...
class UniteCreatorObjectsListView extends UniteElementsBaseUC{
	...
	protected function putHtml_beforeTable(){
		...
		$this->objTable->putSearchForm($textSearch, __("Clear",BLOXBUILDER_TEXTDOMAIN), $isPutSearchForm);
	...
	protected function putHtml_table(){
		...
		$txtDelete = __("Are you sure you want to delete this", BLOXBUILDER_TEXTDOMAIN)." ".$this->txtSingleName;
		...
		?>
			<div id="uc_table_objects_wrapper" data-options="<?php echo $strOptions?>">
				...
				<div>
					<?php 
						$txtNotFound = __("No",BLOXBUILDER_TEXTDOMAIN)." ".$this->txtMultipleName." ".__("Found",BLOXBUILDER_TEXTDOMAIN);
						echo $txtNotFound;
					?>
				</div>			
				<?php else:?>
				...
				<table id="uc_table_objects" class='unite_table_items' data-text-confirm-delete="<?php echo $txtDelete?>">
					<thead>
						<tr>
							<th width=''>
								<?php $this->objTable->putTableOrderHeader("title", $this->txtSingleName." ".__("Title", BLOXBUILDER_TEXTDOMAIN)) ?>
							</th>
							
							<?php if($this->enableCategories == true):?>
							<th width='200'><?php $objTable->putTableOrderHeader("catid", __("Category",BLOXBUILDER_TEXTDOMAIN)) ?>
							<?php endif?>
						...
						<tr class="<?php echo $rowClass?>">
							...
							<td>
								<a href='<?php echo $editLink?>' class="unite-button-primary float_left mleft_15"><?php echo __("Edit", BLOXBUILDER_TEXTDOMAIN)." ".$this->txtSingleName ?></a>
								
								<a href='javascript:void(0)' data-action="delete" data-id="<?php echo $id?>" class="uc-button-action unite-button-secondary float_left mleft_15"><?php _e("Delete",BLOXBUILDER_TEXTDOMAIN); ?></a>
