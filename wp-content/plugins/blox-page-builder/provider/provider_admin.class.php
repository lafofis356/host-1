...
defined('BLOXBUILDER_INC') or die('Restricted access');
   ...
   class UniteProviderAdminUC extends UniteCreatorAdmin{
		...
		protected function isInsidePlugin(){
			...
			if($page == GlobalsUC::PLUGIN_NAME || 
			   strpos($page, GlobalsUC::PLUGIN_NAME."_") !== false || 
			   strpos($page, GlobalsUC::PLUGIN_NAME."-") !== false )
			return(true);
		...
		private static function a_________MENU________(){}
		
		/**
		 *
		 * add menu page
		 */
		protected static function addMenuPage($title,$pageFunctionName,$icon=null,$link=null,$menuSlug=null){
		 ...
		 */
		public static function addAdminMenu_putActually($arrMenuPages, $arrSubMenuPages){
			...
			foreach($arrMenuPages as $mainMenu){
				...
				if(empty($menuSlug) && !empty($pluginName))
					$menuSlug = GlobalsUC::PLUGIN_NAME."-".$pluginName;
				
				if(empty($menuSlug))
					$menuSlug = GlobalsUC::PLUGIN_NAME;
				...
				add_menu_page( $title, $title, self::$capability, $menuSlug, array(self::$t, $pageFunctionName), $icon );
			...
			foreach($arrSubMenuPages as $key=>$submenuMenu){
				...
				if(empty($parentSlug) && !empty($pluginName))
					$parentSlug = GlobalsUC::PLUGIN_NAME."-".$pluginName;
				
				if(empty($parentSlug))
					$parentSlug = GlobalsUC::PLUGIN_NAME;
				...
				add_submenu_page($parentSlug, $title, $title, self::$capability, $slug, array(self::$t, $pageFunctionName) );
		...
		public static function addAdminMenu(){
			...
			//add blox menu
			self::addAdminMenu_putActually(self::$arrMenuPages, self::$arrSubMenuPages);
			...
			self::addAdminMenu_putActually($arrMenuPages, $arrSubMenuPages);
		...
		*/
		public function checkRedirectLink() {
			...
			if($page !== GlobalsUC::PLUGIN_NAME."_gounlimited")
				return(false);
...
<body class="uc-blank-preview uc-view-<?php echo $view?>">
	<?php echo $htmlBody?>
	<?php 	
		echo $htmlJSScripts;		
	...
	?>
		...
		protected function addAdminMenuLinks(){
			...
			$isCreatorExists = $this->isCreatorPluginExists();
			...
			$this->addSubMenuPage($this->coreAddonsView, __('My Addons',BLOXBUILDER_TEXTDOMAIN), "adminPages");
			$this->addSubMenuPage("library", __('My Library',BLOXBUILDER_TEXTDOMAIN), "adminPages");
			
			$this->addSubMenuPage("layouts_general", __('My Layouts',BLOXBUILDER_TEXTDOMAIN), "adminPages");
			
			//if($isCreatorExists == true)
			//	$this->addSubMenuPage("assets", __('Assets Manager',BLOXBUILDER_TEXTDOMAIN), "adminPages");
			
			$this->addSubMenuPage("settings", __('General Settings',BLOXBUILDER_TEXTDOMAIN), "adminPages");
			
			$this->addSubMenuPage("layouts_settings", __('Layouts Settings',BLOXBUILDER_TEXTDOMAIN), "adminPages");
