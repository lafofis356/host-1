...
class UniteCreatorSettings extends UniteCreatorSettingsWork{
	...
	protected function addPostsListPicker($name,$value,$title,$extra){
		...
		$params["description"] = __("Select which Post Type or Custom Post Type you wish to display", BLOXBUILDER_TEXTDOMAIN);
		
		$this->addSelect($name."_posttype", $arrTypesSimple, __("Post Type", BLOXBUILDER_TEXTDOMAIN), $postType, $params);
		...
		$params["description"] = __("Filter Posts by Specific Category", BLOXBUILDER_TEXTDOMAIN);
		$this->addMultiSelect($name."_category", $arrCats, __("Post Category", BLOXBUILDER_TEXTDOMAIN), $category, $params);
		...
		$this->addTextBox($name."_maxitems", $maxItems, __("Max Posts", BLOXBUILDER_TEXTDOMAIN), $params);
		...
		$params["origtype"] = UniteCreatorDialogParam::PARAM_DROPDOWN;
		$params["description"] = __("Select how you wish to order posts", BLOXBUILDER_TEXTDOMAIN);
		
		$this->addSelect($name."_orderby", $arrOrder, __("Order By", BLOXBUILDER_TEXTDOMAIN), $orderBY, $params);
		
		$params = array();
		$params["origtype"] = UniteCreatorDialogParam::PARAM_DROPDOWN;
		
		$params["description"] = __("Select order direction. Descending A-Z or Accending Z-A", BLOXBUILDER_TEXTDOMAIN);
